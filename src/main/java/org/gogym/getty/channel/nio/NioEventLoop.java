 ...
 * under the License.
 */
package org.gogym.getty.channel.nio;
...
/**
 * {@link SingleThreadEventLoop} implementation which register the
 * {@link Channel}'s to a {@link Selector} and so does the multi-plexing of
 * these in the event loop.
 *
 */
public final class NioEventLoop extends SingleThreadEventLoop {
	...
	private static final boolean DISABLE_KEY_SET_OPTIMIZATION = SystemPropertyUtil
	...
	private SelectorTuple openSelector() {
					...
					public Object run() {
						try {
							...
							if (PlatformDependent.javaVersion() >= 9
								...
								// the SelectionKeySet.
								// This allows us to also do this in Java9+
								// without any extra flags.
								long selectedKeysFieldOffset = PlatformDependent
										.objectFieldOffset(selectedKeysField);
								long publicSelectedKeysFieldOffset = PlatformDependent
										.objectFieldOffset(publicSelectedKeysField);
								...
								// We could not retrieve the offset, lets try
							...
							selectedKeysField.set(unwrappedSelector,
									selectedKeySet);
							publicSelectedKeysField.set(unwrappedSelector,
									selectedKeySet);
							return null;
		...
		// unwrappedSelector);
		return new SelectorTuple(unwrappedSelector,
				new SelectedSelectionKeySetSelector(unwrappedSelector,
						selectedKeySet));
	}

	/**
	 * Returns the {@link SelectorProvider} used by this {@link NioEventLoop} to
	 * obtain the {@link Selector}.
	 */
	...
	private static Queue<Runnable> newTaskQueue0(int maxPendingTasks) {
		...
		return maxPendingTasks == Integer.MAX_VALUE ? PlatformDependent
				.<Runnable> newMpscQueue() : PlatformDependent
				.<Runnable> newMpscQueue(maxPendingTasks);
	}

	/**
	 * Registers an arbitrary {@link SelectableChannel}, not necessarily created
	 * by Netty, to the {@link Selector} of this event loop. Once the specified
	 * {@link SelectableChannel} is registered, the specified {@code task} will
	 * be executed by this event loop when the {@link SelectableChannel} is
	 * ready.
	 */
	public void register(final SelectableChannel ch, final int interestOps,
		...
		if ((interestOps & ~ch.validOps()) != 0) {
			throw new IllegalArgumentException("invalid interestOps: "
					+ interestOps + "(validOps: " + ch.validOps() + ')');
		}
		if (task == null) {
		...
		if (inEventLoop()) {
			...
			try {
				...
				// may block for a long time while trying to obtain an internal
				// lock that may be hold while selecting.
				submit(new Runnable() {
					@Override
					public void run() {
						register0(ch, interestOps, task);
				...
				// Even if interrupted we did schedule it so just mark the
	...
	private void register0(SelectableChannel ch, int interestOps,
			NioTask<?> task) {
		try {
	 ...
	 * event loop. The default value is {@code 50}, which means the event loop
	 * will try to spend the same amount of time for I/O as for non-I/O tasks.
	 */
	...
	/**
	 * Replaces the current {@link Selector} of this event loop with newly
	 * created {@link Selector}s to work around the infamous epoll 100% CPU bug.
	 */
	...
	private void rebuildSelector0() {
		...
		for (SelectionKey key : oldSelector.keys()) {
			...
			try {
				if (!key.isValid()
						|| key.channel().keyFor(
								newSelectorTuple.unwrappedSelector) != null) {
					continue;
				}
				...
				// logger.warn(
				// "Failed to re-register a Channel to the new Selector.",
				// e);
				if (a instanceof AbstractNioChannel) {
			...
			try {
				try {
					...
					case SelectStrategy.SELECT:
						...
						// The race condition is triggered when 'wakenUp' is set
						// to
						// true too early.
						//
						// 'wakenUp' is set to true too early if:
						// 1) Selector is waken up between 'wakenUp.set(false)'
						// and
						// 'selector.select(...)'. (BAD)
						...
						// unnecessarily.
						//
						// To fix this problem, we wake up the selector again if
						// wakenUp
					...
					// Selector is messed up. Let's rebuild
	...
	private void processSelectedKey(SelectionKey k, AbstractNioChannel ch) {
		...
		if (!k.isValid()) {
			...
			try {
				...
				// If the channel implementation throws an exception because
				// there is no event loop, we ignore this
				// because we are only trying to determine if ch is registered
				// to this event loop and thus has authority
				// to close ch.
				return;
			}
			// Only close ch if ch is still registered to this EventLoop. ch
			// could have deregistered from the event loop
	...
	private static void processSelectedKey(SelectionKey k,
	...
	private static void invokeChannelUnregistered(
		...
		try {
			...
			// logger.warn("Unexpected exception while running NioTask.channelUnregistered()",
			// e);
		}
	}
		...
		try {
			...
			for (;;) {
				...
				if (time - TimeUnit.MILLISECONDS.toNanos(timeoutMillis) >= currentTimeNanos) {
					...
					// The code exists in an extra method to ensure the method
					// is not too big to inline as this
