...
public final class DefaultChannelId implements ChannelId {
	...
	// InternalLoggerFactory.getInstance(DefaultChannelId.class);
	...
	static {
		...
		if (customProcessId != null) {
			...
			if (processId < 0) {
				processId = -1;
				// logger.warn("-Dio.netty.processId: {} (malformed)",
				// customProcessId);
			}
			// else if (logger.isDebugEnabled()) {
			// logger.debug("-Dio.netty.processId: {} (user-set)",
			// processId);
			// }
		}
		...
		if (processId < 0) {
			...
			// logger.debug("-Dio.netty.processId: {} (auto-detected)",
			// processId);
			// }
		}
		...
		if (customMachineId != null) {
			try {
				...
				// logger.warn("-Dio.netty.machineId: {} (malformed)",
				// customMachineId, e);
			}
			if (machineId != null) {
				// logger.debug("-Dio.netty.machineId: {} (user-set)",
				// customMachineId);
			}
		}
		...
		if (machineId == null) {
			...
			// if (logger.isDebugEnabled()) {
			// logger.debug("-Dio.netty.machineId: {} (auto-detected)",
			// MacAddressUtil.formatAddress(machineId));
			// }
		}
	...
	private static int defaultProcessId() {
		...
		try {
			...
			Class<?> runtimeMxBeanType = Class.forName(
					"java.lang.management.RuntimeMXBean", true, loader);

			Method getRuntimeMXBean = mgmtFactoryType.getMethod(
					"getRuntimeMXBean", EmptyArrays.EMPTY_CLASSES);
			Object bean = getRuntimeMXBean.invoke(null,
					EmptyArrays.EMPTY_OBJECTS);
			Method getName = runtimeMxBeanType.getMethod("getName",
					EmptyArrays.EMPTY_CLASSES);
			value = (String) getName.invoke(bean, EmptyArrays.EMPTY_OBJECTS);
		} catch (Throwable t) {
			// logger.debug("Could not invoke ManagementFactory.getRuntimeMXBean().getName(); Android?",
			// t);
			try {
				// Invoke android.os.Process.myPid()
				Class<?> processType = Class.forName("android.os.Process",
						true, loader);
				Method myPid = processType.getMethod("myPid",
						EmptyArrays.EMPTY_CLASSES);
				value = myPid.invoke(null, EmptyArrays.EMPTY_OBJECTS)
						.toString();
			} catch (Throwable t2) {
				// logger.debug("Could not invoke Process.myPid(); not Android?",
				// t2);
				value = "";
			}
		}
	...
	private DefaultChannelId() {
		data = new byte[MACHINE_ID.length + PROCESS_ID_LEN + SEQUENCE_LEN
				+ TIMESTAMP_LEN + RANDOM_LEN];
		int i = 0;
	...
	public String asShortText() {
		...
		if (shortValue == null) {
			this.shortValue = shortValue = ByteBufUtil.hexDump(data,
					data.length - RANDOM_LEN, RANDOM_LEN);
		}
		return shortValue;
	}
