...
import static org.gogym.getty.channel.ChannelHandlerMask.MASK_BIND;
import static org.gogym.getty.channel.ChannelHandlerMask.MASK_CHANNEL_ACTIVE;
import static org.gogym.getty.channel.ChannelHandlerMask.MASK_CHANNEL_INACTIVE;
import static org.gogym.getty.channel.ChannelHandlerMask.MASK_CHANNEL_READ;
import static org.gogym.getty.channel.ChannelHandlerMask.MASK_CHANNEL_READ_COMPLETE;
import static org.gogym.getty.channel.ChannelHandlerMask.MASK_CHANNEL_REGISTERED;
import static org.gogym.getty.channel.ChannelHandlerMask.MASK_CHANNEL_UNREGISTERED;
import static org.gogym.getty.channel.ChannelHandlerMask.MASK_CHANNEL_WRITABILITY_CHANGED;
import static org.gogym.getty.channel.ChannelHandlerMask.MASK_CLOSE;
import static org.gogym.getty.channel.ChannelHandlerMask.MASK_CONNECT;
import static org.gogym.getty.channel.ChannelHandlerMask.MASK_DEREGISTER;
import static org.gogym.getty.channel.ChannelHandlerMask.MASK_DISCONNECT;
import static org.gogym.getty.channel.ChannelHandlerMask.MASK_EXCEPTION_CAUGHT;
import static org.gogym.getty.channel.ChannelHandlerMask.MASK_FLUSH;
import static org.gogym.getty.channel.ChannelHandlerMask.MASK_READ;
...
public abstract class AbstractChannelHandlerContext implements
	...
	public static void invokeExceptionCaught(final AbstractChannelHandlerContext next,
		...
		if (executor.inEventLoop()) {
			...
			try {
				...
				// t);
				// logger.warn("The exceptionCaught() event that was failed to submit was:",
				// cause);
				// }
			}
		}
	...
	private void invokeExceptionCaught(final Throwable cause) {
		if (invokeHandler()) {
			try {
				...
				// +
				// "was thrown by a user handler's exceptionCaught() " +
				// "method while handling the following exception:", error,
				// cause);
				// }
			}
		} else {
	...
	public ChannelHandlerContext fireUserEventTriggered(final Object event) {
		invokeUserEventTriggered(findContextInbound(MASK_USER_EVENT_TRIGGERED),
				event);
		return this;
	}
	...
	public ChannelHandlerContext flush() {
		...
		if (executor.inEventLoop()) {
			...
			safeExecute(executor, tasks.invokeFlushTask, channel()
					.voidPromise(), null);
		}
	...
	private void write(Object msg, boolean flush, ChannelPromise promise) {
		...
		final AbstractChannelHandlerContext next = findContextOutbound(flush ? (MASK_WRITE | MASK_FLUSH)
				: MASK_WRITE);
		...
		if (executor.inEventLoop()) {
			...
			if (!safeExecute(executor, task, promise, m)) {
				// We failed to submit the AbstractWriteTask. We need to cancel
	...
	public final boolean setAddComplete() {
		for (;;) {
			...
			if (HANDLER_STATE_UPDATER.compareAndSet(this, oldState,
					ADD_COMPLETE)) {
				return true;
			}
		}
	...
	private static boolean safeExecute(EventExecutor executor,
			Runnable runnable, ChannelPromise promise, Object msg) {
		try {
	...
	static final class WriteTask extends AbstractWriteTask implements
	...
	static final class WriteAndFlushTask extends AbstractWriteTask {
		...
		static WriteAndFlushTask newInstance(AbstractChannelHandlerContext ctx,
				Object msg, ChannelPromise promise) {
