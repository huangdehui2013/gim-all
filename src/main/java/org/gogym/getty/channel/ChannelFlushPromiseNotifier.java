...
/**
 * This implementation allows to register {@link ChannelFuture} instances which
 * will get notified once some amount of data was written and so a checkpoint
 * was reached.
 */
public final class ChannelFlushPromiseNotifier {
	 ...
	 *
	 * @param tryNotify
	 *            if {@code true} the {@link ChannelPromise}s will get notified
	 *            with {@link ChannelPromise#trySuccess()} and
	 *            {@link ChannelPromise#tryFailure(Throwable)}. Otherwise
	 *            {@link ChannelPromise#setSuccess()} and
	 *            {@link ChannelPromise#setFailure(Throwable)} is used
	 */
	...
	/**
	 * Create a new instance which will use {@link ChannelPromise#setSuccess()}
	 * and {@link ChannelPromise#setFailure(Throwable)} to notify the
	 * {@link ChannelPromise}s.
	 */
	 ...
	 */
	@Deprecated
	public ChannelFlushPromiseNotifier add(ChannelPromise promise,
			int pendingDataSize) {
		return add(promise, (long) pendingDataSize);
	}

	/**
	 * Add a {@link ChannelPromise} to this {@link ChannelFlushPromiseNotifier}
	 * which will be notified after the given {@code pendingDataSize} was
	 * reached.
	 */
	public ChannelFlushPromiseNotifier add(ChannelPromise promise,
		...
		if (promise instanceof FlushCheckpoint) {
			...
			flushCheckpoints
					.add(new DefaultFlushCheckpoint(checkpoint, promise));
		}
		return this;
	}
	 ...
	 * {@link ChannelFlushPromiseNotifier}
	 */
	...
	/**
	 * Notify all {@link ChannelFuture}s that were registered with
	 * {@link #add(ChannelPromise, int)} and their pendingDatasize is smaller
	 * after the current writeCounter returned by {@link #writeCounter()}.
	 *
	 * After a {@link ChannelFuture} was notified it will be removed from this
	 * {@link ChannelFlushPromiseNotifier} and so not receive anymore
	 * notification.
	 */
	...
	/**
	 * Notify all {@link ChannelFuture}s that were registered with
	 * {@link #add(ChannelPromise, int)} and their pendingDatasize isis smaller
	 * then the current writeCounter returned by {@link #writeCounter()}.
	 *
	 * After a {@link ChannelFuture} was notified it will be removed from this
	 * {@link ChannelFlushPromiseNotifier} and so not receive anymore
	 * notification.
	 *
	 * The rest of the remaining {@link ChannelFuture}s will be failed with the
	 ...
	 * Notify all {@link ChannelFuture}s that were registered with
	 * {@link #add(ChannelPromise, int)} and their pendingDatasize is smaller
	 * then the current writeCounter returned by {@link #writeCounter()} using
	 * the given cause1.
	 *
	 * After a {@link ChannelFuture} was notified it will be removed from this
	 * {@link ChannelFlushPromiseNotifier} and so not receive anymore
	 ...
	 *
	 * @param cause1
	 *            the {@link Throwable} which will be used to fail all of the
	 *            {@link ChannelFuture}s which pendingDataSize is smaller then
	 *            the current writeCounter returned by {@link #writeCounter()}
	 * @param cause2
	 *            the {@link Throwable} which will be used to fail the remaining
	 *            {@link ChannelFuture}s
	 */
	public ChannelFlushPromiseNotifier notifyPromises(Throwable cause1,
			Throwable cause2) {
	 ...
	 */
	@Deprecated
	public ChannelFlushPromiseNotifier notifyFlushFutures(Throwable cause1,
			Throwable cause2) {
	...
	private void notifyPromises0(Throwable cause) {
		...
		for (;;) {
			...
			if (cp == null) {
				// Reset the counter if there's nothing in the notification
				// list.
				this.writeCounter = 0;
				break;
			}
		...
		if (newWriteCounter >= 0x8000000000L) {
			...
			// so that we can reduce the cost of updating all entries in the
			// notification list.
			this.writeCounter = 0;
