...
/**
 * Executes {@link Runnable} objects in the caller's thread. If the
 * {@link #execute(Runnable)} is reentrant it will be queued until the original
 * {@link Runnable} finishes execution.
 * <p>
 * All {@link Throwable} objects thrown from {@link #execute(Runnable)} will be
 * swallowed and logged. This is to ensure that all queued {@link Runnable}
 * objects have the chance to be run.
 */
public final class ImmediateEventExecutor extends AbstractEventExecutor {
	// private static final InternalLogger logger =
	// InternalLoggerFactory.getInstance(ImmediateEventExecutor.class);
	public static final ImmediateEventExecutor INSTANCE = new ImmediateEventExecutor();
	/**
	 * A Runnable will be queued if we are executing a Runnable. This is to
	 * prevent a {@link StackOverflowError}.
	 */
	...
	public void execute(Runnable command) {
		...
		if (!RUNNING.get()) {
			...
			try {
				...
				// logger.info("Throwable caught while executing Runnable {}",
				// command, cause);
			} finally {
