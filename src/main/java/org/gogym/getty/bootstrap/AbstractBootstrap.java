...
import org.gogym.getty.channel.ChannelFactory;
...
import org.gogym.getty.channel.DefaultChannelPromise;
import org.gogym.getty.channel.EventLoop;
import org.gogym.getty.channel.EventLoopGroup;
import org.gogym.getty.channel.ReflectiveChannelFactory;
...
public abstract class AbstractBootstrap<B extends AbstractBootstrap<B, C>, C extends Channel>
	 ...
	 * {@link #channelFactory(org.gogym.getty.channel.ChannelFactory)} if your
	...
	/**
	 * @deprecated Use
	 *             {@link #channelFactory(org.gogym.getty.channel.ChannelFactory)}
	 *             instead.
	 */
	@Deprecated
	 ...
	 * {@link Channel} instances once they got created. Use a value of
	 * {@code null} to remove a previous set {@link ChannelOption}.
	 */
	 ...
	 * {@link Channel}. If the {@code value} is {@code null}, the attribute of
	 * the specified {@code key} is removed.
	 */
	...
	/**
	 * Returns a deep clone of this bootstrap which has the identical
	 * configuration. This method is useful when making multiple {@link Channel}
	...
	private ChannelFuture doBind(final SocketAddress localAddress) {
		...
		if (regFuture.isDone()) {
			// At this point we know that the registration was complete and
			// successful.
			ChannelPromise promise = channel.newPromise();
			doBind0(regFuture, channel, localAddress, promise);
			return promise;
		} else {
			// Registration future is almost always fulfilled already, but just
			// in case it's not.
			final PendingRegistrationPromise promise = new PendingRegistrationPromise(
					channel);
			regFuture.addListener(new ChannelFutureListener() {
				...
				public void operationComplete(ChannelFuture future)
					...
					if (cause != null) {
						...
						// Registration was successful, so set the correct
						// executor to use.
	...
	final ChannelFuture initAndRegister() {
		...
		try {
			...
			if (channel != null) {
				...
				// as the Channel is not registered yet we need to force the
				// usage of the GlobalEventExecutor
				return new DefaultChannelPromise(channel,
						GlobalEventExecutor.INSTANCE).setFailure(t);
			}
			// as the Channel is not registered yet we need to force the usage
			// of the GlobalEventExecutor
		...
		// If we are here and the promise is not failed, it's one of the
		// following cases:
		// 1) If we attempted registration from the event loop, the registration
		// has been completed at this point.
		// i.e. It's safe to attempt bind() or connect() now because the channel
		// has been registered.
		// 2) If we attempted registration from the other thread, the
		// registration request has been successfully
	...
	private static void doBind0(final ChannelFuture regFuture,
		...
		// This method is invoked before channelRegistered() is triggered. Give
		// user handlers a chance to set up
	...
	/**
	 * Returns the {@link AbstractBootstrapConfig} object that can be used to
	 * obtain the current config of the bootstrap.
	 */
	...
	@SuppressWarnings("unchecked")
	private static void setChannelOption(Channel channel,
			ChannelOption<?> option, Object value) {
		try {
			if (!channel.config().setOption((ChannelOption<Object>) option,
					value)) {
				// logger.warn("Unknown channel option '{}' for channel '{}'",
				// option, channel);
			}
		} catch (Throwable t) {
			// logger.warn(
	...
	static final class PendingRegistrationPromise extends DefaultChannelPromise {
		...
		// Is set to the correct EventExecutor once the registration was
		...
		protected EventExecutor executor() {
			...
			// The registration failed so we can only use the
			// GlobalEventExecutor as last resort to notify.
