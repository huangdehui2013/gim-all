...
import org.gogym.getty.buffer.unpooled.UnpooledUnsafeNoCleanerDirectByteBuf;
...
public final class UnsafeByteBufUtil {
	...
	public static short getShort(long address) {
		...
		return (short) (PlatformDependent.getByte(address) << 8 | PlatformDependent
				.getByte(address + 1) & 0xff);
	}
	...
	public static short getShortLE(long address) {
		...
		return (short) (PlatformDependent.getByte(address) & 0xff | PlatformDependent
				.getByte(address + 1) << 8);
	}
	...
	public static int getUnsignedMedium(long address) {
		if (UNALIGNED) {
			return (PlatformDependent.getByte(address) & 0xff) << 16
					| (BIG_ENDIAN_NATIVE_ORDER ? PlatformDependent
							.getShort(address + 1) : Short
							.reverseBytes(PlatformDependent
									.getShort(address + 1))) & 0xffff;
		}
		return (PlatformDependent.getByte(address) & 0xff) << 16
	...
	public static int getUnsignedMediumLE(long address) {
		if (UNALIGNED) {
			return (PlatformDependent.getByte(address) & 0xff)
					| ((BIG_ENDIAN_NATIVE_ORDER ? Short
							.reverseBytes(PlatformDependent
									.getShort(address + 1)) : PlatformDependent
							.getShort(address + 1)) & 0xffff) << 8;
		}
		return PlatformDependent.getByte(address) & 0xff
				| (PlatformDependent.getByte(address + 1) & 0xff) << 8
	...
	public static void setShort(long address, int value) {
		if (UNALIGNED) {
			PlatformDependent.putShort(
					address,
					BIG_ENDIAN_NATIVE_ORDER ? (short) value : Short
							.reverseBytes((short) value));
		} else {
	...
	public static void setShortLE(long address, int value) {
		if (UNALIGNED) {
			PlatformDependent.putShort(address,
					BIG_ENDIAN_NATIVE_ORDER ? Short.reverseBytes((short) value)
							: (short) value);
		} else {
			PlatformDependent.putByte(address, (byte) value);
	...
	public static void setInt(long address, int value) {
		if (UNALIGNED) {
			PlatformDependent.putInt(address, BIG_ENDIAN_NATIVE_ORDER ? value
	...
	public static void setIntLE(long address, int value) {
		if (UNALIGNED) {
			PlatformDependent.putInt(address,
					BIG_ENDIAN_NATIVE_ORDER ? Integer.reverseBytes(value)
							: value);
		} else {
			PlatformDependent.putByte(address, (byte) value);
			PlatformDependent.putByte(address + 1, (byte) (value >>> 8));
	...
	public static void setLong(long address, long value) {
		if (UNALIGNED) {
			PlatformDependent.putLong(address, BIG_ENDIAN_NATIVE_ORDER ? value
	...
	public static void setLongLE(long address, long value) {
		if (UNALIGNED) {
			PlatformDependent.putLong(address,
					BIG_ENDIAN_NATIVE_ORDER ? Long.reverseBytes(value) : value);
		} else {
			PlatformDependent.putByte(address, (byte) value);
			PlatformDependent.putByte(address + 1, (byte) (value >>> 8));
			PlatformDependent.putByte(address + 2, (byte) (value >>> 16));
	...
	public static short getShort(byte[] array, int index) {
		...
		return (short) (PlatformDependent.getByte(array, index) << 8 | PlatformDependent
				.getByte(array, index + 1) & 0xff);
	}
	...
	public static short getShortLE(byte[] array, int index) {
		...
		return (short) (PlatformDependent.getByte(array, index) & 0xff | PlatformDependent
				.getByte(array, index + 1) << 8);
	}

	public static int getUnsignedMedium(byte[] array, int index) {
		if (UNALIGNED) {
			return (PlatformDependent.getByte(array, index) & 0xff) << 16
					| (BIG_ENDIAN_NATIVE_ORDER ? PlatformDependent.getShort(
							array, index + 1) : Short
							.reverseBytes(PlatformDependent.getShort(array,
									index + 1))) & 0xffff;
		}
		return (PlatformDependent.getByte(array, index) & 0xff) << 16
	...
	public static int getUnsignedMediumLE(byte[] array, int index) {
		if (UNALIGNED) {
			return (PlatformDependent.getByte(array, index) & 0xff)
					| ((BIG_ENDIAN_NATIVE_ORDER ? Short
							.reverseBytes(PlatformDependent.getShort(array,
									index + 1)) : PlatformDependent.getShort(
							array, index + 1)) & 0xffff) << 8;
		}
		return PlatformDependent.getByte(array, index) & 0xff
				| (PlatformDependent.getByte(array, index + 1) & 0xff) << 8
	...
	public static void setInt(byte[] array, int index, int value) {
		if (UNALIGNED) {
			PlatformDependent.putInt(
					array,
					index,
					BIG_ENDIAN_NATIVE_ORDER ? value : Integer
							.reverseBytes(value));
		} else {
	...
	public static void setIntLE(byte[] array, int index, int value) {
		if (UNALIGNED) {
			PlatformDependent.putInt(array, index,
					BIG_ENDIAN_NATIVE_ORDER ? Integer.reverseBytes(value)
							: value);
		} else {
			PlatformDependent.putByte(array, index, (byte) value);
	...
	public static void setLong(byte[] array, int index, long value) {
		if (UNALIGNED) {
			PlatformDependent.putLong(array, index,
					BIG_ENDIAN_NATIVE_ORDER ? value : Long.reverseBytes(value));
		} else {
	...
	public static ByteBuf copy(AbstractByteBuf buf, long addr, int index,
			int length) {
		...
		if (length != 0) {
			if (copy.hasMemoryAddress()) {
				PlatformDependent
						.copyMemory(addr, copy.memoryAddress(), length);
	...
	public static int setBytes(AbstractByteBuf buf, long addr, int index,
	...
	public static void getBytes(AbstractByteBuf buf, long addr, int index,
			ByteBuf dst, int dstIndex, int length) {
		buf.checkIndex(index, length);
		...
		if (dst.hasMemoryAddress()) {
			PlatformDependent.copyMemory(addr, dst.memoryAddress() + dstIndex,
					length);
		} else if (dst.hasArray()) {
			PlatformDependent.copyMemory(addr, dst.array(), dst.arrayOffset()
					+ dstIndex, length);
		} else {
	...
	public static void getBytes(AbstractByteBuf buf, long addr, int index,
		...
		if (dst.isDirect()) {
			if (dst.isReadOnly()) {
				// We need to check if dst is ready-only so we not write
				// something in it by using Unsafe.
				throw new ReadOnlyBufferException();
			}
			// Copy to direct memory
			long dstAddress = PlatformDependent.directBufferAddress(dst);
			PlatformDependent.copyMemory(addr, dstAddress + dst.position(),
					dst.remaining());
	...
	public static void setBytes(AbstractByteBuf buf, long addr, int index,
			ByteBuf src, int srcIndex, int length) {
		buf.checkIndex(index, length);
		...
		if (length != 0) {
			if (src.hasMemoryAddress()) {
				PlatformDependent.copyMemory(src.memoryAddress() + srcIndex,
						addr, length);
			} else if (src.hasArray()) {
				PlatformDependent.copyMemory(src.array(), src.arrayOffset()
						+ srcIndex, addr, length);
			} else {
	...
	public static void setBytes(AbstractByteBuf buf, long addr, int index,
			byte[] src, int srcIndex, int length) {
		buf.checkIndex(index, length);
		if (length != 0) {
	...
	public static void setBytes(AbstractByteBuf buf, long addr, int index,
		...
		if (src.isDirect()) {
			...
			PlatformDependent.copyMemory(srcAddress + src.position(), addr,
					length);
			...
			PlatformDependent.copyMemory(src.array(),
					src.arrayOffset() + src.position(), addr, length);
			src.position(src.position() + length);
		} else {
			if (length < 8) {
				setSingleBytes(buf, addr, index, src, length);
			} else {
				// no need to checkIndex: internalNioBuffer is already taking
				// care of it
				assert buf.nioBufferCount() == 1;
				final ByteBuffer internalBuffer = buf.internalNioBuffer(index,
						length);
	...
	private static void setSingleBytes(final AbstractByteBuf buf,
			final long addr, final int index, final ByteBuffer src,
			final int length) {
		buf.checkIndex(index, length);
	...
	public static void getBytes(AbstractByteBuf buf, long addr, int index,
			OutputStream out, int length) throws IOException {
		buf.checkIndex(index, length);
		if (length != 0) {
			int len = Math.min(length, ByteBufUtil.WRITE_CHUNK_SIZE);
			if (len <= ByteBufUtil.MAX_TL_ARRAY_LEN
					|| !buf.alloc().isDirectBufferPooled()) {
				getBytes(addr, ByteBufUtil.threadLocalTempArray(len), 0, len,
						out, length);
			} else {
				// if direct buffers are pooled chances are good that heap
				// buffers are pooled as well.
	...
	private static void getBytes(long inAddr, byte[] in, int inOffset,
			int inLen, OutputStream out, int outLen) throws IOException {
		do {
	...
	public static UnpooledUnsafeDirectByteBuf newUnsafeDirectByteBuf(
			ByteBufAllocator alloc, int initialCapacity, int maxCapacity) {
		if (PlatformDependent.useDirectBufferNoCleaner()) {
			return new UnpooledUnsafeNoCleanerDirectByteBuf(alloc,
					initialCapacity, maxCapacity);
		}
		return new UnpooledUnsafeDirectByteBuf(alloc, initialCapacity,
				maxCapacity);
	}
