...
public final class AdvancedLeakAwareByteBuf extends SimpleLeakAwareByteBuf {
	...
	// InternalLoggerFactory.getInstance(AdvancedLeakAwareByteBuf.class);

	static {
		ACQUIRE_AND_RELEASE_ONLY = SystemPropertyUtil.getBoolean(
				PROP_ACQUIRE_AND_RELEASE_ONLY, false);

		// if (logger.isDebugEnabled()) {
		// logger.debug("-D{}: {}", PROP_ACQUIRE_AND_RELEASE_ONLY,
		// ACQUIRE_AND_RELEASE_ONLY);
		// }

		ResourceLeakDetector.addExclusions(AdvancedLeakAwareByteBuf.class,
	...
	@Override
	protected AdvancedLeakAwareByteBuf newLeakAwareByteBuf(ByteBuf buf,
			ByteBuf trackedByteBuf, ResourceLeakTracker<ByteBuf> leakTracker) {
