...
/**
 * {@link ByteBuf} implementation which allows to wrap an array of
 * {@link ByteBuf} in a read-only mode. This is useful to write an array of
 * {@link ByteBuf}s.
 */
public final class FixedCompositeByteBuf extends
	...
	private Component findComponent(int index) {
		...
		for (int i = 0; i < buffers.length; i++) {
			...
			if (index < readable) {
				if (comp == null) {
					// Create a new component and store it in the array so it
	...
	public int _getUnsignedMedium(int index) {
		...
		if (index + 3 <= c.endOffset) {
			...
			return _getShort(index) & 0xFFFF
					| (_getByte(index + 2) & 0xFF) << 16;
		}
	}
	...
	public int _getUnsignedMediumLE(int index) {
		...
		if (index + 3 <= c.endOffset) {
			...
			return _getShortLE(index) & 0xffff
					| (_getByte(index + 2) & 0xff) << 16;
		} else {
	...
	public int _getInt(int index) {
		...
		if (index + 4 <= c.endOffset) {
			...
			return (_getShort(index) & 0xffff) << 16 | _getShort(index + 2)
					& 0xffff;
		} else {
			return _getShort(index) & 0xFFFF
					| (_getShort(index + 2) & 0xFFFF) << 16;
		}
	}
	...
	public int _getIntLE(int index) {
		...
		if (index + 4 <= c.endOffset) {
			...
			return _getShortLE(index) & 0xFFFF
					| (_getShortLE(index + 2) & 0xFFFF) << 16;
		} else {
			return (_getShortLE(index) & 0xffff) << 16 | _getShortLE(index + 2)
					& 0xffff;
		}
	}
	...
	public long _getLong(int index) {
		...
		if (index + 8 <= c.endOffset) {
			...
			return (_getInt(index) & 0xffffffffL) << 32 | _getInt(index + 4)
					& 0xffffffffL;
		} else {
			return _getInt(index) & 0xFFFFFFFFL
					| (_getInt(index + 4) & 0xFFFFFFFFL) << 32;
		}
	}
	...
	public long _getLongLE(int index) {
		...
		if (index + 8 <= c.endOffset) {
			...
			return _getIntLE(index) & 0xffffffffL
					| (_getIntLE(index + 4) & 0xffffffffL) << 32;
		} else {
			return (_getIntLE(index) & 0xffffffffL) << 32
					| _getIntLE(index + 4) & 0xffffffffL;
		}
	}
	...
	public ByteBuffer[] nioBuffers(int index, int length) {
		...
		RecyclableArrayList array = RecyclableArrayList
				.newInstance(buffers.length);
		try {
			Component c = findComponent(index);
			int i = c.index;
			int adjustment = c.offset;
