...
public abstract class AbstractUnpooledSlicedByteBuf extends
	...
	public AbstractUnpooledSlicedByteBuf(ByteBuf buffer, int index, int length) {
		...
		if (buffer instanceof AbstractUnpooledSlicedByteBuf) {
			...
			adjustment = ((AbstractUnpooledSlicedByteBuf) buffer).adjustment
					+ index;
	 ...
	 * 
	 * @param length
	 *            the {@code length} argument from the constructor.
	 */
	...
	public ByteBuf duplicate() {
		return unwrap().duplicate().setIndex(idx(readerIndex()),
				idx(writerIndex()));
	}
	...
	static void checkSliceOutOfBounds(int index, int length, ByteBuf buffer) {
		if (isOutOfBounds(index, length, buffer.capacity())) {
			throw new IndexOutOfBoundsException(buffer + ".slice(" + index
					+ ", " + length + ')');
		}
	}
}
