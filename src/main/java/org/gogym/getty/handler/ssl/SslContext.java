...
import org.gogym.getty.buffer.ByteBufInputStream;
...
public abstract class SslContext {
	...
	static {
		try {
			...
			throw new IllegalStateException(
					"unable to instance X.509 CertificateFactory", e);
		}
	}
	 ...
	 *
	 * @param provider
	 *            the {@link SslContext} implementation to use. {@code null} to
	 *            use the current default one.
	 * @param certChainFile
	 *            an X.509 certificate chain file in PEM format
	 * @param keyFile
	 *            a PKCS#8 private key file in PEM format
	 ...
	 *            {@link TrustManager}s that verifies the certificates sent from
	 *            servers. {@code null} to use the default.
	 * @param ciphers
	 *            the cipher suites to enable, in the order of preference.
	 *            {@code null} to use the default cipher suites.
	 * @param nextProtocols
	...
	public static SslContext newServerContext(SslProvider provider,
		...
		return newServerContext(provider, null, trustManagerFactory,
				certChainFile, keyFile, keyPassword, null, ciphers,
	 ...
	 *            {@code null} to use the default cipher suites.
	 * @param cipherFilter
	 *            a filter to apply over the supplied list of ciphers Only
	 *            required if {@code provider} is {@link SslProvider#JDK}
	 * @param apn
	 *            Provides a means to configure parameters related to
	 *            application protocol negotiation.
	 * @param sessionCacheSize
	...
	public static SslContext newServerContext(SslProvider provider,
			...
			long sessionTimeout) throws SSLException {
		return newServerContext(provider, null, null, certChainFile, keyFile,
				keyPassword, null, ciphers, cipherFilter, apn,
	 ...
	 *            an X.509 certificate collection file in PEM format. This
	 *            provides the certificate collection used for mutual
	 *            authentication. {@code null} to use the system default
	 * @param trustManagerFactory
	 *            the {@link TrustManagerFactory} that provides the
	 *            {@link TrustManager}s that verifies the certificates sent from
	 *            clients. {@code null} to use the default or the results of
	 *            parsing {@code trustCertCollectionFile}. This parameter is
	 *            ignored if {@code provider} is not {@link SslProvider#JDK}.
	 ...
	 *            clients. {@code null} to use the default or the results of
	 *            parsing {@code keyCertChainFile} and {@code keyFile}. This
	 *            parameter is ignored if {@code provider} is not
	...
	public static SslContext newServerContext(SslProvider provider,
			...
			CipherSuiteFilter cipherFilter, ApplicationProtocolConfig apn,
			long sessionCacheSize, long sessionTimeout) throws SSLException {
		return newServerContext(provider, trustCertCollectionFile,
				trustManagerFactory, keyCertChainFile, keyFile, keyPassword,
				keyManagerFactory, ciphers, cipherFilter, apn,
				sessionCacheSize, sessionTimeout, KeyStore.getDefaultType());
	}
	 ...
	 *            {@code 0} to use the default value.
	 * @param keyStore
	 *            the keystore type that should be used
	 * @return a new server-side {@link SslContext}
	 */
	static SslContext newServerContext(SslProvider provider,
		...
		try {
			...
			throw new SSLException(
					"failed to initialize the server-side SSL context", e);
		}
	}

	static SslContext newServerContextInternal(SslProvider provider,
			Provider sslContextProvider, X509Certificate[] trustCertCollection,
		...
		case OPENSSL:
			...
			return new OpenSslServerContext(trustCertCollection,
					trustManagerFactory, keyCertChain, key, keyPassword,
					keyManagerFactory, ciphers, cipherFilter, apn,
					sessionCacheSize, sessionTimeout, clientAuth, protocols,
					startTls, enableOcsp, keyStoreType);
		case OPENSSL_REFCNT:
			verifyNullSslContextProvider(provider, sslContextProvider);
			return new ReferenceCountedOpenSslServerContext(
					trustCertCollection, trustManagerFactory, keyCertChain,
					key, keyPassword, keyManagerFactory, ciphers, cipherFilter,
					apn, sessionCacheSize, sessionTimeout, clientAuth,
					protocols, startTls, enableOcsp, keyStoreType);
		default:
			throw new Error(provider.toString());
		}
	}

	private static void verifyNullSslContextProvider(SslProvider provider,
			Provider sslContextProvider) {
		if (sslContextProvider != null) {
			throw new IllegalArgumentException(
					"Java Security Provider unsupported for SslProvider: "
							+ provider);
		}
	}
	 ...
	 *
	 * @param certChainFile
	 *            an X.509 certificate chain file in PEM format. {@code null} to
	...
	@Deprecated
	public static SslContext newClientContext(SslProvider provider,
			File certChainFile) throws SSLException {
	...
	public static SslContext newClientContext(SslProvider provider,
			...
			throws SSLException {
		return newClientContext(provider, certChainFile, trustManagerFactory,
				null, IdentityCipherSuiteFilter.INSTANCE, null, 0, 0);
	}
	...
	public static SslContext newClientContext(SslProvider provider,
		...
		return newClientContext(provider, certChainFile, trustManagerFactory,
				null, null, null, null, ciphers, cipherFilter, apn,
	 ...
	 *            the {@link TrustManagerFactory} that provides the
	 *            {@link TrustManager}s that verifies the certificates sent from
	 *            servers. {@code null} to use the default or the results of
	 *            parsing {@code trustCertCollectionFile}. This parameter is
	 *            ignored if {@code provider} is not {@link SslProvider#JDK}.
	 * @param keyCertChainFile
	 *            an X.509 certificate chain file in PEM format. This provides
	 *            the public key for mutual authentication. {@code null} to use
	 *            the system default
	 * @param keyFile
	 ...
	 * @param keyPassword
	 *            the password of the {@code keyFile}. {@code null} if it's not
	 *            password-protected. Ignored if {@code keyFile} is {@code null}
	 *            .
	 * @param keyManagerFactory
	 *            the {@link KeyManagerFactory} that provides the
	 *            {@link KeyManager}s that is used to encrypt data being sent to
	...
	public static SslContext newClientContext(SslProvider provider,
		...
		try {
			return newClientContextInternal(provider, null,
					...
					trustManagerFactory, toX509Certificates(keyCertChainFile),
					toPrivateKey(keyFile, keyPassword), keyPassword,
					keyManagerFactory, ciphers, cipherFilter, apn, null,
					sessionCacheSize, sessionTimeout, false,
			...
			throw new SSLException(
					"failed to initialize the client-side SSL context", e);
		}
	}

	static SslContext newClientContextInternal(SslProvider provider,
			Provider sslContextProvider, X509Certificate[] trustCert,
			TrustManagerFactory trustManagerFactory,
			X509Certificate[] keyCertChain, PrivateKey key, String keyPassword,
			KeyManagerFactory keyManagerFactory, Iterable<String> ciphers,
			CipherSuiteFilter cipherFilter, ApplicationProtocolConfig apn,
		...
		case OPENSSL:
			verifyNullSslContextProvider(provider, sslContextProvider);
			return new OpenSslClientContext(trustCert, trustManagerFactory,
					keyCertChain, key, keyPassword, keyManagerFactory, ciphers,
					cipherFilter, apn, protocols, sessionCacheSize,
					sessionTimeout, enableOcsp, keyStoreType);
		case OPENSSL_REFCNT:
			verifyNullSslContextProvider(provider, sslContextProvider);
			return new ReferenceCountedOpenSslClientContext(trustCert,
					trustManagerFactory, keyCertChain, key, keyPassword,
					keyManagerFactory, ciphers, cipherFilter, apn, protocols,
					sessionCacheSize, sessionTimeout, enableOcsp, keyStoreType);
		default:
			throw new Error(provider.toString());
		}
	}
...
//	}
	 ...
	 * leak native memory!
	 * <p>
	 * <b>Beware</b>: the underlying generated {@link SSLEngine} won't have <a
	 * href="https://wiki.openssl.org/index.php/Hostname_validation">hostname
	 ...
	 */
	public final SslHandler newHandler(ByteBufAllocator alloc, String peerHost,
			int peerPort) {
	 ...
	 * @see #newHandler(ByteBufAllocator, String, int, boolean, Executor)
	 */
	protected SslHandler newHandler(ByteBufAllocator alloc, String peerHost,
			int peerPort, boolean startTls) {
	 ...
	 * @return a new {@link SslHandler}
	 */
	public SslHandler newHandler(ByteBufAllocator alloc, String peerHost,
			int peerPort, Executor delegatedTaskExecutor) {
		return newHandler(alloc, peerHost, peerPort, startTls,
				delegatedTaskExecutor);
	}

	protected SslHandler newHandler(ByteBufAllocator alloc, String peerHost,
			int peerPort, boolean startTls, Executor delegatedTaskExecutor) {
		return new SslHandler(newEngine(alloc, peerHost, peerPort), startTls,
				delegatedTaskExecutor);
	}
	...
	protected static PKCS8EncodedKeySpec generateKeySpec(char[] password,
			byte[] key) throws IOException, NoSuchAlgorithmException,
	 ...
	 *            password-protected.
	 * @param keyStoreType
	 *            The KeyStore Type you want to use
	...
	static PrivateKey toPrivateKey(File keyFile, String keyPassword)
		...
		return getPrivateKeyFromByteBuffer(PemReader.readPrivateKey(keyFile),
				keyPassword);
	}

	static PrivateKey toPrivateKey(InputStream keyInputStream,
		...
		return getPrivateKeyFromByteBuffer(
				PemReader.readPrivateKey(keyInputStream), keyPassword);
	}

	private static PrivateKey getPrivateKeyFromByteBuffer(
			ByteBuf encodedKeyBuf, String keyPassword)
			throws NoSuchAlgorithmException, NoSuchPaddingException,
			InvalidKeySpecException, InvalidAlgorithmParameterException,
			KeyException, IOException {
			...
			try {
				...
				try {
					...
					throw new InvalidKeySpecException(
							"Neither RSA, DSA nor EC worked", e);
				}
			}
		}
	}
	 ...
	 * @param trustManagerFactory
	 *            The existing {@link TrustManagerFactory} that will be used if
	 *            not {@code null}.
	...
	protected static TrustManagerFactory buildTrustManagerFactory(
		...
		return buildTrustManagerFactory(certChainFile, trustManagerFactory,
				KeyStore.getDefaultType());
	}
	 ...
	 *         {@code certChainFile}
	 */
	static TrustManagerFactory buildTrustManagerFactory(File certChainFile,
			TrustManagerFactory trustManagerFactory, String keyType)
			throws NoSuchAlgorithmException, CertificateException,
			KeyStoreException, IOException {
	...
	static TrustManagerFactory buildTrustManagerFactory(
			...
			TrustManagerFactory trustManagerFactory, String keyStoreType)
			throws NoSuchAlgorithmException, CertificateException,
			KeyStoreException, IOException {
		if (keyStoreType == null) {
			keyStoreType = KeyStore.getDefaultType();
	...
	static KeyManagerFactory buildKeyManagerFactory(
			X509Certificate[] certChain, PrivateKey key, String keyPassword,
			KeyManagerFactory kmf, String keyStoreType)
			throws UnrecoverableKeyException, KeyStoreException,
			NoSuchAlgorithmException, CertificateException, IOException {
		return buildKeyManagerFactory(certChain,
				KeyManagerFactory.getDefaultAlgorithm(), key, keyPassword, kmf,
				keyStoreType);
	}

	static KeyManagerFactory buildKeyManagerFactory(
			X509Certificate[] certChainFile, String keyAlgorithm,
			PrivateKey key, String keyPassword, KeyManagerFactory kmf,
			String keyStore) throws KeyStoreException,
	...
	static KeyManagerFactory buildKeyManagerFactory(KeyStore ks,
			String keyAlgorithm, char[] keyPasswordChars, KeyManagerFactory kmf)
			throws KeyStoreException, NoSuchAlgorithmException,
			UnrecoverableKeyException {
	...
	static char[] keyStorePassword(String keyPassword) {
		return keyPassword == null ? EmptyArrays.EMPTY_CHARS : keyPassword
				.toCharArray();
	}
}
