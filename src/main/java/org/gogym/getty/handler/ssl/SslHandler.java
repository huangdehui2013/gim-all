...
import static org.gogym.getty.buffer.ByteBufUtil.ensureWritableSuccess;
import static org.gogym.getty.handler.ssl.SslUtils.getEncryptedPacketLength;
...
import org.gogym.getty.channel.AbstractCoalescingBufferQueue;
...
import org.gogym.getty.channel.ChannelPromiseNotifier;
import org.gogym.getty.handler.codec.ByteToMessageDecoder;
import org.gogym.getty.handler.codec.DecoderException;
import org.gogym.getty.handler.codec.UnsupportedMessageTypeException;
...
import org.gogym.getty.util.concurrent.ImmediateExecutor;
import org.gogym.getty.util.concurrent.Promise;
import org.gogym.getty.util.concurrent.PromiseNotifier;
import org.gogym.getty.util.internal.PlatformDependent;
import org.gogym.getty.util.internal.UnstableApi;

/**
 * Adds <a href="http://en.wikipedia.org/wiki/Transport_Layer_Security">SSL
 * &middot; TLS</a> and StartTLS support to a {@link Channel}. Please refer to
 * the <strong>"SecureChat"</strong> example in the distribution or the web site
 ...
 * is active and {@link SSLEngine#getUseClientMode()} returns {@code true}. So
 * no need to bother with it by your self.
 *
 ...
 * intercepts the close request and send the {@code close_notify} message before
 * the channel closure automatically. Once the SSL session is closed, it is not
 * reusable, and consequently you should create a new {@link SslHandler} with a
 * new {@link SSLEngine} as explained in the following section.
 *
 * <h3>Restarting the session</h3>
 * <p>
 * To restart the SSL session, you must remove the existing closed
 * {@link SslHandler} from the {@link ChannelPipeline}, insert a new
 * {@link SslHandler} with a new {@link SSLEngine} into the pipeline, and start
 * the handshake process as described in the first section.
 *
 ...
 * secures the wire from the beginning of the connection. Typically, StartTLS is
 ...
 * <h3>Known issues</h3>
 * <p>
 * Because of a known issue with the current implementation of the SslEngine
 * that comes with Java it may be possible that you see blocked IO-Threads while
 * a full GC is done.
 * <p>
 * So if you are affected you can workaround this problem by adjust the cache
 ...
 * </pre>
 * <p>
 * What values to use here depends on the nature of your application and should
 * be set based on monitoring and debugging of it. For more details see <a
 * href="https://github.com/netty/netty/issues/832">#832</a> in our issue
 * tracker.
 */
public class SslHandler extends ByteToMessageDecoder implements
		ChannelOutboundHandler {

//	private static final InternalLogger logger = InternalLoggerFactory
//			.getInstance(SslHandler.class);

	private static final Pattern IGNORABLE_CLASS_IN_STACK = Pattern
			.compile("^.*(?:Socket|Datagram|Sctp|Udt)Channel.*$");
	private static final Pattern IGNORABLE_ERROR_MESSAGE = Pattern.compile(
			"^.*(?:connection.*(?:reset|closed|abort|broken)|broken.*pipe).*$",
			Pattern.CASE_INSENSITIVE);

	/**
	 * <a href="https://tools.ietf.org/html/rfc5246#section-6.2">2^14</a> which
	 * is the maximum sized plaintext chunk allowed by the TLS RFC.
	 */
	...
	private enum SslEngineType {
		TCNATIVE(true, COMPOSITE_CUMULATOR) {
			...
			SSLEngineResult unwrap(SslHandler handler, ByteBuf in,
					int readerIndex, int len, ByteBuf out) throws SSLException {
				int nioBufferCount = in.nioBufferCount();
				int writerIndex = out.writerIndex();
				final SSLEngineResult result;
				if (nioBufferCount > 1) {
					/*
					 * If {@link OpenSslEngine} is in use, we can use a special
					 * {@link OpenSslEngine#unwrap(ByteBuffer[], ByteBuffer[])}
					 * method that accepts multiple {@link ByteBuffer}s without
					...
					result = handler.engine
							.unwrap(toByteBuffer(in, readerIndex, len),
			...
			int calculatePendingData(SslHandler handler, int guess) {
				int sslPending = ((ReferenceCountedOpenSslEngine) handler.engine)
						.sslPending();
		...
		JDK(false, MERGE_CUMULATOR) {
			...
			SSLEngineResult unwrap(SslHandler handler, ByteBuf in,
				...
				final SSLEngineResult result = handler.engine.unwrap(
						inNioBuffer,
						toByteBuffer(out, writerIndex, out.writableBytes()));
				out.writerIndex(writerIndex + result.bytesProduced());

				// This is a workaround for a bug in Android 5.0. Android 5.0
				// does not correctly update the
				// SSLEngineResult.bytesConsumed() in some cases and just return
				// 0.
				//
				// See:
				// -
				...
				if (result.bytesConsumed() == 0) {
					...
					if (consumed != result.bytesConsumed()) {
						// Create a new SSLEngineResult with the correct
						// bytesConsumed().
						return new SSLEngineResult(result.getStatus(),
								result.getHandshakeStatus(), consumed,
		...
		abstract SSLEngineResult unwrap(SslHandler handler, ByteBuf in,
				int readerIndex, int len, ByteBuf out) throws SSLException;

		abstract int calculateWrapBufferCapacity(SslHandler handler,
				int pendingBytes, int numComponents);
		...
		/**
		 * {@code true} if and only if {@link SSLEngine} expects a direct
		 * buffer.
		 */
		...
		/**
		 * When using JDK {@link SSLEngine}, we use {@link #MERGE_CUMULATOR}
		 * because it works only with one {@link ByteBuffer}.
		 *
		 * When using {@link OpenSslEngine}, we can use
		 * {@link #COMPOSITE_CUMULATOR} because it has
		 * {@link OpenSslEngine#unwrap(ByteBuffer[], ByteBuffer[])} which works
		 * with multiple {@link ByteBuffer}s and which does not need to do extra
		 * memory copies.
		 */
	...
	/**
	 * Used if {@link SSLEngine#wrap(ByteBuffer[], ByteBuffer)} and
	 * {@link SSLEngine#unwrap(ByteBuffer, ByteBuffer[])} should be called with
	 * a {@link ByteBuf} that is only backed by one {@link ByteBuffer} to reduce
	 * the object creation.
	 */
	 ...
	 * {@link #channelReadComplete(ChannelHandlerContext)} will check this flag,
	 * clear it, and call ctx.flush().
	 */
	 ...
	 *            the {@link Executor} that will be used to execute tasks that
	 *            are returned by {@link SSLEngine#getDelegatedTask()}.
	 */
	...
	public void setHandshakeTimeoutMillis(long handshakeTimeoutMillis) {
		if (handshakeTimeoutMillis < 0) {
			throw new IllegalArgumentException("handshakeTimeoutMillis: "
					+ handshakeTimeoutMillis + " (expected: >= 0)");
		}
	 ...
	 * {@link #write(ChannelHandlerContext, Object, ChannelPromise)}. The
	 * partitioning will work as follows:
	 * <ul>
	 * <li>If {@code wrapDataSize <= 0} then we will write each data chunk as
	 * is.</li>
	 * <li>If {@code wrapDataSize > data size} then we will attempt to aggregate
	 * multiple data chunks together.</li>
	 * <li>If {@code wrapDataSize > data size} Else if
	 * {@code wrapDataSize <= data size} then we will divide the data into
	 * chunks of {@code wrapDataSize} when writing.</li>
	 * </ul>
	 * <p>
	 * If the {@link SSLEngine} doesn't support a gather wrap operation (e.g.
	 * {@link SslProvider#OPENSSL}) then aggregating data before wrapping can
	 * help reduce the ratio between TLS overhead vs data payload which will
	 * lead to better goodput. Writing fixed chunks of data can also help target
	 * the underlying transport's (e.g. TCP) frame size. Under lossy/congested
	 * network conditions this may help the peer get full TLS packets earlier
	 * and be able to do work sooner, as opposed to waiting for the all the
	 * pieces of the TLS packet to arrive.
	 * 
	 * @param wrapDataSize
	...
	/**
	 * Gets the timeout for flushing the close_notify that was triggered by
	 * closing the {@link Channel}. If the close_notify was not flushed in the
	 * given timeout the {@link Channel} will be closed forcibly.
	 */
	...
	/**
	 * Sets the timeout for flushing the close_notify that was triggered by
	 * closing the {@link Channel}. If the close_notify was not flushed in the
	 * given timeout the {@link Channel} will be closed forcibly.
	 */
	 ...
	 */
	public final void setCloseNotifyFlushTimeoutMillis(
	...
	/**
	 * Gets the timeout (in ms) for receiving the response for the close_notify
	 * that was triggered by closing the {@link Channel}. This timeout starts
	 * after the close_notify message was successfully written to the remote
	 * peer. Use {@code 0} to directly close the {@link Channel} and not wait
	 * for the response.
	 */
	...
	/**
	 * Sets the timeout for receiving the response for the close_notify that was
	 * triggered by closing the {@link Channel}. This timeout starts after the
	 * close_notify message was successfully written to the remote peer. Use
	 * {@code 0} to directly close the {@link Channel} and not wait for the
	 * response.
	 */
	 ...
	 */
	public final void setCloseNotifyReadTimeoutMillis(
		...
		if (closeNotifyReadTimeoutMillis < 0) {
			throw new IllegalArgumentException("closeNotifyReadTimeoutMillis: "
					+ closeNotifyReadTimeoutMillis + " (expected: >= 0)");
		}
	 ...
	 *
	 * @return the protocol name or {@code null} if application-level protocol
	...
	public String applicationProtocol() {
		...
		return ((ApplicationProtocolAccessor) engine)
				.getNegotiatedApplicationProtocol();
	}

	/**
	 * Returns a {@link Future} that will get notified once the current TLS
	 * handshake completes.
	 *
	 * @return the {@link Future} for the initial TLS handshake if
	 *         {@link #renegotiate()} was not invoked. The {@link Future} for
	 *         the most recent {@linkplain #renegotiate() TLS renegotiation}
	 *         otherwise.
	 */
	 ...
	 * destroys the underlying {@link SSLEngine}. This will <strong>not</strong>
	 * close the underlying {@link Channel}. If you want to also close the
	 * {@link Channel} use {@link Channel#close()} or
...
//				logger.warn("{} flush() raised a masked exception.",
//						ctx.channel(), e);
			}
		}
	}
	...
	/**
	 * Return the {@link Future} that will get notified if the inbound of the
	 * {@link SSLEngine} is closed.
	 *
	...
	public void handlerRemoved0(ChannelHandlerContext ctx) throws Exception {
		if (!pendingUnencryptedWrites.isEmpty()) {
			// Check if queue is not empty first because create a new
			// ChannelException is expensive
	...
	private static IllegalStateException newPendingWritesNullException() {
		return new IllegalStateException(
				"pendingUnencryptedWrites is null, handlerRemoved0 called?");
	}
	...
	public void write(final ChannelHandlerContext ctx, Object msg,
		...
		if (!(msg instanceof ByteBuf)) {
			UnsupportedMessageTypeException exception = new UnsupportedMessageTypeException(
					msg, ByteBuf.class);
	...
	public void flush(ChannelHandlerContext ctx) throws Exception {
		...
		if (startTls && !sentFirstMessage) {
			...
			forceFlush(ctx);
			// Explicit start handshake processing once we send the first
			// message. This will also ensure
	...
	private void wrapAndFlush(ChannelHandlerContext ctx) throws SSLException {
		if (pendingUnencryptedWrites.isEmpty()) {
			...
			// may want to add a ChannelFutureListener to the ChannelPromise
			// later.
			//
		...
		try {
			...
			// We may have written some parts of data before an exception was
			// thrown so ensure we always flush.
			// See
	...
	private void wrap(ChannelHandlerContext ctx, boolean inUnwrap)
		...
		try {
			...
			// Only continue to loop if the handler was not removed in the
			// meantime.
			// See https://github.com/netty/netty/issues/5860
			outer: while (!ctx.isRemoved()) {
				promise = ctx.newPromise();
				buf = wrapDataSize > 0 ? pendingUnencryptedWrites.remove(alloc,
						wrapDataSize, promise) : pendingUnencryptedWrites
						.removeFirst(promise);
				if (buf == null) {
					break;
				}
				...
				if (result.getStatus() == Status.CLOSED) {
					buf.release();
					buf = null;
					SSLException exception = new SSLException(
					...
					if (buf.isReadable()) {
						...
						// When we add the buffer/promise pair back we need to
						// be sure we don't complete the promise
						// later in finishWrap. We only complete the promise if
						// the buffer is completely consumed.
						promise = null;
					} else {
						buf.release();
					}
					...
					case NEED_TASK:
						if (!runDelegatedTasks(inUnwrap)) {
							// We scheduled a task on the
							// delegatingTaskExecutor, so stop processing as we
							// will
	...
	private void finishWrap(ChannelHandlerContext ctx, ByteBuf out,
			ChannelPromise promise, boolean inUnwrap, boolean needUnwrap) {
		if (out == null) {
		...
		if (needUnwrap) {
			// The underlying engine is starving so we need to feed it with more
			// data.
	 ...
	 * or {@link #setHandshakeFailure(ChannelHandlerContext, Throwable)}.
	 * 
	 * @return {@code true} if this method ends on
	 *         {@link SSLEngineResult.HandshakeStatus#NOT_HANDSHAKING}.
	 */
	private boolean wrapNonAppData(final ChannelHandlerContext ctx,
			boolean inUnwrap) throws SSLException {
		ByteBuf out = null;
		ByteBufAllocator alloc = ctx.alloc();
		try {
			// Only continue to loop if the handler was not removed in the
			...
			outer: while (!ctx.isRemoved()) {
				if (out == null) {
					// As this is called for the handshake we have no real idea
					// how big the buffer needs to be.
					// That said 2048 should give us enough room to include
					// everything like ALPN / NPN data.
					// If this is not enough we will increase the buffer in
					// wrap(...).
				...
				case NEED_TASK:
					if (!runDelegatedTasks(inUnwrap)) {
						// We scheduled a task on the delegatingTaskExecutor, so
						// stop processing as we will
						// resume once the task completes.
						break outer;
					}
					break;
				case NEED_UNWRAP:
					if (inUnwrap) {
						// If we asked for a wrap, the engine requested an
						// unwrap, and we are in unwrap there is
						// no use in trying to call wrap again because we have
						// already attempted (or will after we
				...
				// Check if did not produce any bytes and if so break out of the
				// loop, but only if we did not process
				// a task as last action. It's fine to not produce any data as
				// part of executing a task.
				if (result.bytesProduced() == 0
	...
	private SSLEngineResult wrap(ByteBufAllocator alloc, SSLEngine engine,
		...
		try {
			...
			// We will call SslEngine.wrap(ByteBuffer[], ByteBuffer) to allow
			// efficient handling of
			// CompositeByteBuf without force an extra memory copy when
			// CompositeByteBuffer.nioBuffer() is called.
			final ByteBuffer[] in0;
			if (in.isDirect() || !engineType.wantsDirectBuffer) {
				// As CompositeByteBuf.nioBufferCount() can be expensive (as it
				// needs to check all composed ByteBuf
				// to calculate the count) we will just assume a
				// CompositeByteBuf contains more then 1 ByteBuf.
				// The worst that can happen is that we allocate an extra
				// ByteBuffer[] in CompositeByteBuf.nioBuffers()
				// which is better then walking the composed ByteBuf in most
				// cases.
				if (!(in instanceof CompositeByteBuf)
						&& in.nioBufferCount() == 1) {
					in0 = singleBuffer;
					// We know its only backed by 1 ByteBuffer so use
					// internalNioBuffer to keep object allocation
					// to a minimum.
				...
				// We could even go further here and check if its a
				// CompositeByteBuf and if so try to decompose it and
				// only replace the ByteBuffer that are not direct. At the
				// moment we just will replace the whole
				// CompositeByteBuf to keep the complexity to a minimum
				newDirectIn = alloc.directBuffer(readableBytes);
				newDirectIn.writeBytes(in, readerIndex, readableBytes);
				in0 = singleBuffer;
				in0[0] = newDirectIn.internalNioBuffer(
						newDirectIn.readerIndex(), readableBytes);
			}

			for (;;) {
				ByteBuffer out0 = out.nioBuffer(out.writerIndex(),
						out.writableBytes());
				...
				case BUFFER_OVERFLOW:
					out.ensureWritable(engine.getSession()
							.getPacketBufferSize());
					break;
				default:
					return result;
	...
	public void channelInactive(ChannelHandlerContext ctx) throws Exception {
		...
		// and notify the handshake future if the connection has been closed
		// during handshake.
		setHandshakeFailure(ctx, exception, !outboundClosed, handshakeStarted,
				false);
...
//			}
	...
	private boolean ignoreException(Throwable t) {
		if (!(t instanceof SSLException) && t instanceof IOException
			...
			if (message != null
					&& IGNORABLE_ERROR_MESSAGE.matcher(message).matches()) {
				return true;
			}
			...
			for (StackTraceElement element : elements) {
				...
				try {
					...
					if (PlatformDependent.javaVersion() >= 7
							&& "com.sun.nio.sctp.SctpChannel".equals(clazz
									.getSuperclass().getName())) {
						return true;
					}
				} catch (Throwable cause) {
//					logger.debug(
//							"Unexpected exception while loading class {} classname {}",
//							getClass(), classname, cause);
	...
	/**
	 * Returns {@code true} if the given {@link ByteBuf} is encrypted. Be aware
	 * that this method will not increase the readerIndex of the given
	 * {@link ByteBuf}.
	 *
	 * @param buffer
	 *            The {@link ByteBuf} to read from. Be aware that it must have
	 *            at least 5 bytes to read, otherwise it will throw an
	 *            {@link IllegalArgumentException}.
	 * @return encrypted {@code true} if the {@link ByteBuf} is encrypted,
	 *         {@code false} otherwise.
	 * @throws IllegalArgumentException
	 *             Is thrown if the given {@link ByteBuf} has not at least 5
	 *             bytes to read.
	 */
	public static boolean isEncrypted(ByteBuf buffer) {
		if (buffer.readableBytes() < SslUtils.SSL_RECORD_HEADER_LENGTH) {
			throw new IllegalArgumentException("buffer must have at least "
					+ SslUtils.SSL_RECORD_HEADER_LENGTH + " readable bytes");
		}
	...
	private void decodeJdkCompatible(ChannelHandlerContext ctx, ByteBuf in)
		...
		if (packetLength > 0) {
			...
			// Get the packet length and wait until we get a packets worth of
			// data to unwrap.
			...
			if (packetLength == SslUtils.NOT_ENCRYPTED) {
				...
				// First fail the handshake promise as we may need to have
				// access to the SSLEngine which may
				// be released because the user will remove the SslHandler in an
				// exceptionCaught(...) implementation.
		...
		// Reset the state of this class so we can get the length of the next
		// packet. We assume the entire packet will
		// be consumed by the SSLEngine.
		this.packetLength = 0;
		try {
			int bytesConsumed = unwrap(ctx, in, in.readerIndex(), packetLength);
			assert bytesConsumed == packetLength || engine.isInboundDone() : "we feed the SSLEngine a packets worth of data: "
					+ packetLength + " but it only consumed: " + bytesConsumed;
	...
	private void handleUnwrapThrowable(ChannelHandlerContext ctx,
		...
		try {
			...
			// listeners immediately close the Channel then we may end up firing
			// the handshake event after the Channel
			// has been closed.
			if (handshakePromise.tryFailure(cause)) {
				ctx.fireUserEventTriggered(new SslHandshakeCompletionEvent(
						cause));
			}

			// We need to flush one time as there may be an alert that we should
			// send to the remote peer because
			// of the SSLException reported here.
			wrapAndFlush(ctx);
		} catch (SSLException ex) {
//			logger.debug(
//					"SSLException during trying to call SSLEngine.wrap(...)"
//							+ " because of an previous SSLException, ignoring...",
//					ex);
		} finally {
	...
	private void readIfNeeded(ChannelHandlerContext ctx) {
		...
		if (!ctx.channel().config().isAutoRead()
				&& (!firedChannelRead || !handshakePromise.isDone())) {
			// No auto-read used and no message passed through the
			// ChannelPipeline or the handshake was not complete
			// yet, which means we need to trigger the read to ensure we not
			// encounter any stalls.
			ctx.read();
		}
	}
	 ...
	 */
	private int unwrap(ChannelHandlerContext ctx, ByteBuf packet, int offset,
		...
		try {
			...
			unwrapLoop: while (!ctx.isRemoved()) {
				final SSLEngineResult result = engineType.unwrap(this, packet,
						offset, length, decodeOut);
				final Status status = result.getStatus();
				final HandshakeStatus handshakeStatus = result
						.getHandshakeStatus();
				...
				case BUFFER_OVERFLOW:
					...
					int bufferSize = engine.getSession()
							.getApplicationBufferSize() - readableBytes;
					if (readableBytes > 0) {
						...
						decodeOut = null;
						if (bufferSize <= 0) {
							// It may happen that readableBytes >=
							// engine.getSession().getApplicationBufferSize()
							// while there is still more to unwrap, in this case
							// we will just allocate a new buffer
							// with the capacity of
							// engine.getSession().getApplicationBufferSize()
							// and call unwrap
							// again.
							bufferSize = engine.getSession()
					...
					if (readableBytes == 0
							&& previousOverflowReadableBytes == 0) {
						// If there is two consecutive loops where we overflow
						// and are not able to consume any data,
						// assume the amount of data exceeds the maximum amount
						// for the engine and bail
						throw new IllegalStateException(
								"Two consecutive overflows but no content was consumed. "
										+ SSLSession.class.getSimpleName()
										+ " getApplicationBufferSize: "
										+ engine.getSession()
												.getApplicationBufferSize()
										+ " maybe too small.");
					}
					// Allocate a new buffer which can hold all the rest data
					// and loop again.
					// TODO: We may want to reconsider how we calculate the
					// length here as we may
					// have more then one ssl message to decode.
					decodeOut = allocate(ctx,
							engineType.calculatePendingData(this, bufferSize));
					continue;
				case CLOSED:
				...
				case NEED_WRAP:
					...
					// unwrap then the next call to unwrap will not produce any
					// data. We can avoid the potentially
				...
				case NEED_TASK:
					if (!runDelegatedTasks(true)) {
						...
						// resume once the task completes.
						//
						// We break out of the loop only and do NOT return here
						// as we still may need to notify
				...
				case FINISHED:
					...
					// We 'break' here and NOT 'continue' as android API version
					// 21 has a bug where they consume
					// data from the buffer but NOT correctly set the
					// SSLEngineResult.bytesConsumed().
					// Because of this it will raise an exception on the next
					// iteration of the for loop on android
					// API version 21. Just doing a break will work here as
					// produced and consumed will both be 0
					// and so we break out of the complete for (;;) loop and so
					// call decode(...) again later on.
					// On other platforms this will have no negative effect as
					// we will just continue with the
				...
				case NOT_HANDSHAKING:
					...
					// If we are not handshaking and there is no more data to
					// unwrap then the next call to unwrap
					// will not produce any data. We can avoid the potentially
					// costly unwrap operation and break
					// out of the loop.
					if (length == 0) {
						break unwrapLoop;
					}
				...
				if (status == Status.BUFFER_UNDERFLOW
						||
						// If we processed NEED_TASK we should try again even we
						// did not consume or produce anything.
			...
			if (flushedBeforeHandshake && handshakePromise.isDone()) {
				// We need to call wrap(...) in case there was a flush done
				// before the handshake completed to ensure
				// we do not stale.
				//
	...
	private static boolean inEventLoop(Executor executor) {
		return executor instanceof EventExecutor
				&& ((EventExecutor) executor).inEventLoop();
	}
	...
	/**
	 * Will either run the delegated task directly calling
	 * {@link Runnable#run()} and return {@code true} or will offload the
	 * delegated task using {@link Executor#execute(Runnable)} and return
	 * {@code false}.
	 *
	 * If the task is offloaded it will take care to resume its work on the
	 * {@link EventExecutor} once there are no more tasks to process.
	 */
	private boolean runDelegatedTasks(boolean inUnwrap) {
		if (delegatedTaskExecutor == ImmediateExecutor.INSTANCE
				|| inEventLoop(delegatedTaskExecutor)) {
			// We should run the task directly in the EventExecutor thread and
			// not offload at all.
	...
	/**
	 * {@link Runnable} that will be scheduled on the
	 * {@code delegatedTaskExecutor} and will take care of resume work on the
	 * {@link EventExecutor} once the task was executed.
	 */
	private final class SslTasksRunner implements Runnable {
		...
		private void taskError(Throwable e) {
			if (inUnwrap) {
				// As the error happened while the task was scheduled as part of
				// unwrap(...) we also need to ensure
				// we fire it through the pipeline as inbound error to be
				// consistent with what we do in decode(...).
				//
				// This will also ensure we fail the handshake future and flush
				// all produced data.
				try {
		...
		private Throwable wrapIfNeeded(Throwable cause) {
			if (!inUnwrap) {
				// If we are not in unwrap(...) we can just rethrow without
				// wrapping at all.
				return cause;
			}
			// As the exception would have been triggered by an inbound
			// operation we will need to wrap it in a
			// DecoderException to mimic what a decoder would do when
			// decode(...) throws.
			return cause instanceof DecoderException ? cause
					: new DecoderException(cause);
		}
		...
		private void tryDecodeAgain() {
			try {
				...
				// As we called channelRead(...) we also need to call
				// channelReadComplete(...) which
				// will ensure we either call ctx.fireChannelReadComplete() or
				// will trigger a ctx.read() if
				// more data is needed.
				channelReadComplete0(ctx);
			}
		}
		...
		/**
		 * Executed after the wrapped {@code task} was executed via
		 * {@code delegatedTaskExecutor} to resume work on the
		 * {@link EventExecutor}.
		 */
		private void resumeOnEventExecutor() {
			...
			try {
				...
				switch (status) {
				// There is another task that needs to be executed and offloaded
				// to the delegatingTaskExecutor.
				case NEED_TASK:
					executeDelegatedTasks(inUnwrap);

					break;

				// The handshake finished, lets notify about the completion of
				// it and resume processing.
				case FINISHED:
					setHandshakeSuccess();

					// deliberate fall-through

					// Not handshaking anymore, lets notify about the completion
					// if not done yet and resume processing.
				case NOT_HANDSHAKING:
					setHandshakeSuccessIfStillHandshaking();
					try {
						// Lets call wrap to ensure we produce the alert if
						// there is any pending and also to
					...
					if (inUnwrap) {
						// If we were in the unwrap call when the task was
						// processed we should also try to unwrap
						// non app data first as there may not anything left in
						// the inbound buffer to process.
						unwrapNonAppData(ctx);
					}

					// Flush now as we may have written some data as part of the
					// wrap call.
					forceFlush(ctx);
				...
				// We need more data so lets try to unwrap first and then call
				// decode again which will feed us
				...
				// To make progress we need to call SSLEngine.wrap(...) which
				// may produce more output data
				// that will be written to the Channel.
				case NEED_WRAP:
					try {
						if (!wrapNonAppData(ctx, false) && inUnwrap) {
							// The handshake finished in wrapNonAppData(...), we
							// need to try call
							// unwrapNonAppData(...) as we may have some alert
							// that we should read.
							//
							// This mimics what we would do when we are calling
							// this method while in unwrap(...).
							unwrapNonAppData(ctx);
						}

						// Flush now as we may have written some data as part of
						// the wrap call.
						forceFlush(ctx);
					} catch (Throwable e) {
						taskError(e);
		...
		private void handleException(final Throwable cause) {
			if (ctx.executor().inEventLoop()) {
				...
				try {
					...
					processTask = false;
					// the context itself will handle the rejected exception
					// when try to schedule the operation so
	...
	/**
	 * Works around some Android {@link SSLEngine} implementations that skip
	 * {@link HandshakeStatus#FINISHED} and go straight into
	 * {@link HandshakeStatus#NOT_HANDSHAKING} when handshake is finished.
	 *
	 * @return {@code true} if and only if the workaround has been applied and
	 *         thus {@link #handshakeFuture} has been marked as success by this
	 *         method
	 */
...
//		if (logger.isDebugEnabled()) {
//			logger.debug("{} HANDSHAKEN: {}", ctx.channel(), engine
...
//		}
	...
	private void setHandshakeFailure(ChannelHandlerContext ctx,
			Throwable cause, boolean closeInbound, boolean notify,
...
//					if (logger.isDebugEnabled()) {
//						// only log in debug mode as it most likely harmless and
//						// latest chrome still trigger
//						// this all the time.
//						//
//						// See https://github.com/netty/netty/issues/1340
//						String msg = e.getMessage();
//						if (msg == null
//								|| !(msg.contains("possible truncation attack") || msg
//										.contains("closing inbound before receiving peer's close_notify"))) {
//							logger.debug(
//									"{} SSLEngine.closeInbound() raised an exception.",
//									ctx.channel(), e);
//						}
//					}
				}
			...
			// Ensure we remove and fail all pending writes in all cases and so
	...
	private void setHandshakeFailureTransportFailure(ChannelHandlerContext ctx,
			Throwable cause) {
		// If TLS control frames fail to write we are in an unknown state and
		// may become out of
		// sync with our peer. We give up and close the channel. This will also
		// take care of
		// cleaning up any outstanding state (e.g. handshake promise, queued
		// unencrypted data).
		try {
			SSLException transportFailure = new SSLException(
					"failure when writing TLS control frames", cause);
	...
	private void closeOutboundAndChannel(final ChannelHandlerContext ctx,
			final ChannelPromise promise, boolean disconnect) throws Exception {
		outboundClosed = true;
		engine.closeOutbound();
		...
		try {
			...
			if (!closeNotify) {
				closeNotify = true;
				// It's important that we do not pass the original
				// ChannelPromise to safeClose(...) as when flush(....)
				// throws an Exception it will be propagated to the
				// AbstractChannelHandlerContext which will try
				// to fail the promise because of this. This will then fail as
				// it was already completed by
				// safeClose(...). We create a new ChannelPromise and try to
				// notify the original ChannelPromise
				// once it is complete. If we fail to do so we just ignore it as
				// in this case it was failed already
				...
				safeClose(
						ctx,
						closeNotifyPromise,
						ctx.newPromise().addListener(
								new ChannelPromiseNotifier(false, promise)));
			} else {
				// / We already handling the close_notify so just attach the
				// promise to the sslClosePromise.
	...
	public void handlerAdded(final ChannelHandlerContext ctx) throws Exception {
		...
		pendingUnencryptedWrites = new SslHandlerCoalescingBufferQueue(
				ctx.channel(), 16);
		if (ctx.channel().isActive()) {
			startHandshakeProcessing();
		}
	...
	private void renegotiateOnEventLoop(
			final Promise<Channel> newHandshakePromise) {
		final Promise<Channel> oldHandshakePromise = handshakePromise;
		if (!oldHandshakePromise.isDone()) {
			// There's no need to handshake because handshake is in progress
			// already.
			// Merge the new promise into the old one.
			oldHandshakePromise
					.addListener(new PromiseNotifier<Channel, Future<Channel>>(
							newHandshakePromise));
		} else {
	...
	private void handshake() {
		if (engine.getHandshakeStatus() != HandshakeStatus.NOT_HANDSHAKING) {
			// Not all SSLEngine implementations support calling beginHandshake
			// multiple times while a handshake
			// is in progress. See https://github.com/netty/netty/issues/4718.
			return;
		} else {
			if (handshakePromise.isDone()) {
				// If the handshake is done already lets just return directly as
				// there is no need to trigger it again.
				// This can happen if the handshake(...) was triggered before we
				// called channelActive(...) by a
				// flush() that was triggered by a ChannelFutureListener that
				// was added to the ChannelFuture returned
				// from the connect(...) method. In this case we will see the
				// flush() happen before we had a chance to
					...
					public void run() {
						...
						try {
							if (localHandshakePromise.tryFailure(exception)) {
								SslUtils.handleHandshakeFailure(ctx, exception,
										true);
							}
						} finally {
	...
	private void safeClose(final ChannelHandlerContext ctx,
			final ChannelFuture flushFuture, final ChannelPromise promise) {
		if (!ctx.channel().isActive()) {
			ctx.close(promise);
			return;
		}
		...
		if (!flushFuture.isDone()) {
			...
			if (closeNotifyTimeout > 0) {
				// Force-close the connection if close_notify is not fully sent
				// in time.
				timeoutFuture = ctx.executor().schedule(new Runnable() {
					@Override
					public void run() {
						// May be done in the meantime as cancel(...) is only
						// best effort.
						if (!flushFuture.isDone()) {
//							logger.warn(
//									"{} Last write attempt timed out; force-closing the connection.",
//									ctx.channel());
							addCloseListener(ctx.close(ctx.newPromise()),
									promise);
						}
					}
		...
		flushFuture.addListener(new ChannelFutureListener() {
			...
			public void operationComplete(ChannelFuture f) throws Exception {
				...
				if (closeNotifyReadTimeout <= 0) {
					// Trigger the close in all cases to make sure the promise
					// is notified
...
//													closeNotifyReadTimeout);
	...
	private static void addCloseListener(ChannelFuture future,
		...
		// We notify the promise in the ChannelPromiseNotifier as there is a
		// "race" where the close(...) call
		// by the timeoutFuture and the close call in the flushFuture listener
		// will be called. Because of
		// this we need to use trySuccess() and tryFailure(...) as otherwise we
		// can cause an
		// IllegalStateException.
		// Also we not want to log if the notification happens as this is
	...
	/**
	 * Always prefer a direct buffer when it's pooled, so that we reduce the
	 * number of memory copies in {@link OpenSslEngine}.
	 */
	 ...
	 * {@link SSLEngine#wrap(ByteBuffer, ByteBuffer)} which can encrypt the
	 * specified amount of pending bytes.
	 */
	private ByteBuf allocateOutNetBuf(ChannelHandlerContext ctx,
			int pendingBytes, int numComponents) {
		return allocate(ctx, engineType.calculateWrapBufferCapacity(this,
				pendingBytes, numComponents));
	}

	/**
	 * Each call to SSL_write will introduce about ~100 bytes of overhead. This
	 * coalescing queue attempts to increase goodput by aggregating the
	 * plaintext in chunks of {@link #wrapDataSize}. If many small chunks are
	 * written this can increase goodput, decrease the amount of calls to
	 * SSL_write, and decrease overall encryption operations.
	 */
	private final class SslHandlerCoalescingBufferQueue extends
		...
		@Override
		protected ByteBuf compose(ByteBufAllocator alloc, ByteBuf cumulation,
				ByteBuf next) {
			...
			if (cumulation instanceof CompositeByteBuf) {
				...
				if (numComponents == 0
						|| !attemptCopyToCumulation(
								composite.internalComponent(numComponents - 1),
								next, wrapDataSize)) {
	...
	private static boolean attemptCopyToCumulation(ByteBuf cumulation,
		...
		// Avoid using the same buffer if next's data would make cumulation
		// exceed the wrapDataSize.
		// Only copy if there is enough space available and the capacity is
		// large enough, and attempt to
		// resize if the capacity is small.
				(cumulation.isWritable(inReadableBytes)
						&& cumulationCapacity >= wrapDataSize || cumulationCapacity < wrapDataSize
						&& ensureWritableSuccess(cumulation.ensureWritable(
								inReadableBytes, false)))) {
	...
	private final class LazyChannelPromise extends DefaultPromise<Channel> {
		...
		protected void checkDeadLock() {
			if (ctx == null) {
				// If ctx is null the handlerAdded(...) callback was not called,
				// in this case the checkDeadLock()
				// method was called from another Thread then the one that is
				// used by ctx.executor(). We need to
				// guard against this as a user can see a race if
				// handshakeFuture().sync() is called but the
				// handlerAdded(..) method was not yet as it is called from the
				// EventExecutor of the
				// ChannelHandlerContext. If we not guard against this
				// super.checkDeadLock() would cause an
